...
.login-page {
  ...
  background: radial-gradient(ellipse at top, rgba(37,99,235,0.15) 0%, transparent 60%), var(--bg);
  padding: 24px;
}
.login-card {
...
.sidebar-link.active { background: rgba(37,99,235,0.15); color: var(--primary); }
.sidebar-footer {
  padding: 16px 10px;
...
.data-item-actions { display: flex; gap: 8px; flex-shrink: 0; }
...
.confirm-box p { font-size: 0.875rem; color: var(--text-muted); margin-bottom: 24px; }
...
@media (max-width: 768px) {
  ...
  .data-item-actions .btn span { display: none; }
  .mobile-menu-btn {
    display: flex !important;
  }
}
.mobile-menu-btn { display: none; align-items: center; }
.sidebar-backdrop {
  display: none;
  position: fixed; inset: 0;
...
.empty-list i { font-size: 2rem; margin-bottom: 8px; display: block; opacity: 0.4; }
.empty-list p { font-size: 0.875rem; }
