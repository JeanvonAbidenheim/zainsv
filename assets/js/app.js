...
import { initTheme, setStatStripValues } from "./theme.js";
...
function initNavbar() {
  ...
  // Close mobile menu on link click (toggle button itself is handled in theme.js initMobileNav)
...
async function loadSkills() {
  ...
  try {
    ...
    snap.forEach(docSnap => {
      ...
      card.innerHTML = `
        <img class="skill-logo" src="${d.logoUrl || `https://skillicons.dev/icons?i=${d.name.toLowerCase()}`}" alt="${d.name}" onerror="this.src='https://via.placeholder.com/52x52/F2EFEA/948B7E?text=${d.name[0]}'" />
...
async function loadDemoData() {
  ...
  setStatStripValues({
    ...
    certs: demoCerts.length,
    skills: demoSkills.length,
    exp: demoExp.length + demoWork.length,
  });
}
...
export async function initApp() {
  typeRole();
  initTheme();   // Mega menu, mobile nav, stat counter, scroll progress
  initNavbar();
  initReveal();
...
// ---- Hitung & set nilai Stat Strip dari koleksi Firestore ----
async function updateStatStrip() {
  try {
    const [p, c, s, e, w] = await Promise.all([
    ...
    console.warn("Stat strip failed:", e.message);
  }
}
